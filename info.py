...
API_ID = int(environ.get('API_ID', '26368249'))
API_HASH = environ.get('API_HASH', 'ac2db262d5eb7ce0efc0162404c8d172')
BOT_TOKEN = environ.get('BOT_TOKEN', "7172159049:AAHEZF6FPEwseSnQoQZ1uxcS2Cdsq_QgAYQ")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002094378906'))
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6917654808').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002094378906').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []

# auth_channel means force subscribe channel.
auth_channel = environ.get('AUTH_CHANNEL', '-1002094378906') # give your force subscribe channel id here else leave it blank
auth_grp = environ.get('AUTH_GROUP', '-1001954208654') # give your force subscribe group id here else leave it blank
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001954208654')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001954208654')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None
INDEX_REQ_CHANNEL = int(environ.get('INDEX_REQ_CHANNEL', LOG_CHANNEL))
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL','-1002094378906')).split()]
...
OWNER_USERNAME = environ.get('OWNER_USERNAME', 'LPRPDV_Official1') # owner username without @
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'LPRPDV_Official1') # Support Chat Link Without https:// or @
